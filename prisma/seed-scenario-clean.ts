...
async function seedSampleScenario() {
  ...
  try {
    ...
    const occupiedRooms = ['101', '105', '201', '206', '301'];
    ...
    const maintenanceRooms = ['103', '210'];
    ...
    await prisma.room.update({
      where: { roomNumber: '112' },
      data: {
        status: 'OUT_OF_ORDER',
        description:
          'Room 112 temporarily out of order. Plumbing repair required.',
      },
    });
    console.log('✅ Set 1 room as OUT_OF_ORDER');

    // Update some room prices for special offers
    const premiumRooms = ['311', '310', '312'];
    ...
    console.log(
      `   📈 Occupancy Rate: ${Math.round((statusCounts[1] / 36) * 100)}%`,
    );
  } catch (error) {
